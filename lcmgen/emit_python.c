...
static void emit_python_fromjson(const lcmgen_t *lcm, FILE *f, lcm_struct_t *ls)
{
    emit(1, "def from_dict(self, j):");
    for (unsigned int m = 0; m < ls->members->len; m++) {
        lcm_member_t *lm = (lcm_member_t *) g_ptr_array_index(ls->members, m);
        if(! lcm_is_primitive_type(lm->type->lctypename))
        {
        ...
        {
            emit(2, "self.%s = j[\"%s\"]", lm->membername, lm->membername);
        }
    }
    fprintf(f, "\n");
}

static void emit_python_tojson(const lcmgen_t *lcm, FILE *f, lcm_struct_t *ls)
{
    emit(1, "def to_dict(self):");
    emit(2, "j = {}");
    for (unsigned int m = 0; m < ls->members->len; m++) {
        lcm_member_t *lm = (lcm_member_t *) g_ptr_array_index(ls->members, m);
        if(! lcm_is_primitive_type(lm->type->lctypename))
...
//            g_mkdir_with_parents (package_dir, 0755);
    ...
    for (int i = 0; i < pc->structs->len; i++) {
        ...
        emit_python_fromjson(lcm, f, ls);
        emit_python_tojson(lcm, f, ls);
        fclose(f);
    }
